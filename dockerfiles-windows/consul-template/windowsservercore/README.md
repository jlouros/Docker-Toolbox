...
docker run -e CONSUL_HTTP_ADDR=https://my.consul.server -e FEATURE_FLAG_CONSUL_PATH=keyValue/path/inConsul -e FEATURE_FLAG_FILE_PATH=C:\path\inside\container\output.json consul-template

# (example 2) run container, consul-template runs only once
docker run -e CONSUL_HTTP_ADDR=https://my.consul.server -e FEATURE_FLAG_CONSUL_PATH=keyValue/path/inConsul -e FEATURE_FLAG_FILE_PATH=C:\path\inside\container\output.json consul-template -once
```
